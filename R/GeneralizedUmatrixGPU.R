GeneralizedUmatrixGPU=function(Data, ProjectedPoints, PlotIt=FALSE, Cls=NULL,
  ...
  if(is.null(dots[["Radii"]])){
    RadiusVector=pmax(seq(from=AnfangsRadius-1,by=-1,length.out = HeuristischerParameter),1)
  }else{
    RadiusVector=dots$Radii
  }
  ...
  if(isTRUE(PlotIt)){
    ...
    gplotres=TopviewTopographicMap(GeneralizedUmatrix = Umap,
                                   BestMatchingUnits = BMUs,
                                   Cls = Cls, Tiled = Tiled, BmSize = 12) #Sondern Gebirge=Unbekannte Orte der U-Matrix
    print(gplotres)
  }
